...
def melting_issue_query(doctype, txt, searchfield, start, page_len, filters):
    ...
    return frappe.db.sql("""
        SELECT
            ...
            CONCAT(
                'Batch: ',
                COALESCE(mi.batch_no, mi.name),
                ' | Issue: ',
                mi.name,
                ' | Date: ',
                DATE_FORMAT(COALESCE(mi.issue_date, mi.creation), '%%d-%%m-%%Y')
            ) AS description
        FROM `tabMelting Issue` mi
        WHERE mi.docstatus = 1
          {conditions}
          AND NOT EXISTS (
              SELECT 1
              FROM `tabMelting Receive` mr
              WHERE mr.docstatus = 1
                AND mr.melting_issue = mi.name
          )
          AND (
              mi.name LIKE %(txt)s
              OR COALESCE(mi.batch_no, '') LIKE %(txt)s
              OR COALESCE(mi.company, '') LIKE %(txt)s
          )
        ORDER BY mi.creation DESC
