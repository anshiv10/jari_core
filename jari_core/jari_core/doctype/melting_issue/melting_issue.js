...
// BEGIN EXACT STOCK SOURCE SELECTION: Melting Issue

frappe.ui.form.on('Melting Issue', {
    setup(frm) {
        jari_melting_source_setup_source_query(frm);
    },
    ...
    company(frm) {
        jari_melting_source_clear_all_sources(frm);
    }
});
...
frappe.ui.form.on('Melting Issue Item', {
    product(frm, cdt, cdn) {
        jari_melting_source_clear_source_row(
            cdt,
            cdn
        );
    },
    ...
    async stock_source(frm, cdt, cdn) {
        await jari_melting_source_load_source(
            frm,
            cdt,
            cdn
        );
...
function jari_melting_source_setup_source_query(frm) {
    frm.set_query(
        'stock_source',
        'issue_items',
...
async function jari_melting_source_load_source(
    frm,
    cdt,
    cdn
) {
    ...
    if (preferredLocation) {
        ...
        department =
            await jari_melting_source_choose_department(
                locations
            );
    }
...
function jari_melting_source_choose_department(
    locations
) {
...
function jari_melting_source_clear_source_row(
    cdt,
    cdn
) {
    if (
...
function jari_melting_source_clear_all_sources(frm) {
    (
        frm.doc.issue_items
        || []
    ).forEach(row => {
