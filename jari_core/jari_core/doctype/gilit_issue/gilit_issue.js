frappe.ui.form.on('Gilit Issue', {
    refresh(frm) {
        ...
        frm.set_query('spindal_peti_entry', 'peti_items', () => ({
            filters: { docstatus: 1, status: ['!=', 'Fully Consumed'] }
        }));
...
frappe.ui.form.on('Gilit Issue Peti Item', {
        ...
        frappe.db.get_doc('Spindal Peti Entry', row.spindal_peti_entry).then(peti => {
            frappe.call({
                ...
                callback(r) {
                    ...
                    frappe.model.set_value(cdt, cdn, 'available_bobbin', peti.remaining_bobbin || peti.bobbin_count);
                    frappe.model.set_value(cdt, cdn, 'balance_bobbin_after_issue', peti.remaining_bobbin || peti.bobbin_count);
    ...
    issued_bobbin(frm, cdt, cdn) {
        let row = locals[cdt][cdn];
        let available = flt(row.available_bobbin || row.total_bobbin);
